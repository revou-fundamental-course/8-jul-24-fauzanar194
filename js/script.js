...
numberInputs.forEach((input, i) => {
  ...
  input.addEventListener("input", () => {
    // Validasi nilai input apakah memenuhi nilai minimum yang sudah ditentukan
    if (!input.value) {
...
function validation() {
  ...
  $_("input").forEach((input) => {
    // Jika jenis kelamin tidak kosong, kondisi diubah menjadi "true"
    if (input.type === "radio" && input.checked) {
      isRadioChecked = true;
    }
    // Jika form kosong, kondisi diubah menjadi "true"
    if (input.value === "") {
      isInputEmpty = true;
    }
  });
...
$("#button-reset").addEventListener("click", () => {
  ...
  $(".warning").textContent = ""; // Menghapus pesan peringatan
